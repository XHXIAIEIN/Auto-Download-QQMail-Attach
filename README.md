...
1. 如果开启了[按邮件新建文件夹]或[自动重命名]功能，附件名包含空格字符，可能会导致脚本崩溃。 <br> 
...
3. 如果长时间没有在QQ客户端登录过账号，会提示非常用设备异常登录，需要短信验证。 <br> 
> 解决方案：先在QQ客户端登录账号，再运行脚本，登录页面会出现快速登陆的头像。记得勾选"下次自动登陆"，下次就会直接进入邮箱主页了。  
  
4. 只有"超大附件"会过期。并且超大附件(bigattach="1")会有一个参数("timeout"= 0 or 1)，这参数是普通附件(attach="1")没有的。
