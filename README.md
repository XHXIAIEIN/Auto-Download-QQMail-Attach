...
1. 第一次启用脚本时需手动登陆一次，勾选下次自动登陆，然后重启脚本,下一次就会自动登陆直接进入邮箱主页了。不嫌麻烦也可以先运行下面这段代码，手动输入账号密码登陆邮箱,勾选下次自动登陆。
...
2. 把你想要下载的邮件移动到邮箱文件夹里面。
在邮箱左侧的面板选中，右键新窗口打开，从浏览器地址栏中找到几个参数: `sid` `folderid` `page`
https://mail.qq.com/cgi-bin/frame_html?t=frame_html&sid={ A }&url=/cgi-bin/mail_list?folderid={ B }%26page={ C }


> 注：如果邮件数量比较多，建议在邮箱设置-常规里面，调整每页显示100封邮件。
